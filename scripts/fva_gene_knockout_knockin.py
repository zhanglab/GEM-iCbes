...
if __name__ == '__main__':
    ...
    parser.add_argument('--knock-table', help='the input table to present the \
                            gene knockout or reaction knock in; \
                            the first column is the output file name; \
                            the second column is the gene deletion ID; \
                            the third column is the added reaction ID. (sep by space)')
    # parser.add_argument('--rule', action='append',
    #                     help='constraint rule to set, '
    #                          '"ratio1,rxn1,ratio2,rxn2", means flux(rxn1) == '
    #                          'float(ratio1) / float(ratio2) * rxn2_var')
    ...
    parser.add_argument('--final-print', type=argparse.FileType("w"),
                        default=None, help='print the final result table')
    ...
    # read knock table
    with open(args.knock_table, 'r') as t:
        table = csv.reader(t, dialect='excel-tab')
        for row in table:
            mm = native.create_metabolic_model()
            output = row[0]
            with open(output, "w") as final_output:
                gene_delete = row[1]
                rxn_add = row[2]
                ...
                testing_genes = []
                if gene_delete != "--":
                    gene_delete = gene_delete.split(" ")
                    for g in gene_delete:
                ...
                if args.objective is not None:
                    ...
                    if len(obj) == 1:
                        ...
                        logger.info('maximum flux of objective {}: {}'.format(objective_rxns, args.max))
            ...
            # print the final resutl table
            rxn_list = [args.objective, "EX_C00033[e]", "EX_C00022[e]", "EX_C00186[e]", "EX_C00466[e]", "EX_C00011[e]", "EX_C00282[e]", "sink_biomass"]
            args.final_print.write("{}".format(round(p.flux_bound(args.objective, 1), int(args.decimal))))
            for rxn in rxn_list:
                flux_range = '[{}, {}]'.format(round(p.flux_bound(rxn, -1), int(args.decimal)),
                                               round(p.flux_bound(rxn, 1), int(args.decimal)))
                args.final_print.write("\t")
                args.final_print.write(flux_range)
            args.final_print.write("\n")
